    ...
    if convert_type == 'pngtobtx':
        if fileformat == '.png':
            try:
                ...
                print('ERROR: ' + str(e))
                if await get_admin(user_id=message.from_user.id):
                    await message.answer('[Error #100] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>', reply_markup=kb.main_admin)
                else:
                    await message.answer('[Error #100] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>', reply_markup=kb.main)
        else:
            if await get_admin(user_id=message.from_user.id):
        ...
        if fileformat == '.btx':
            try:
                ...
                print('ERROR: ' + str(e))
                if await get_admin(user_id=message.from_user.id):
                    await message.answer('[Error #102] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>', reply_markup=kb.main_admin)
                else:
                    await message.answer('[Error #102] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>', reply_markup=kb.main)
        else:
            if await get_admin(user_id=message.from_user.id):
...
async def convertprocess_pngtobtx(texturename, message: Message):
    ...
    if not texture.PreMultiplyAlpha():
        await message.answer('[Error #301] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>')
    ...
    if not texture.GenerateMIPMaps(pvrpy.ResizeMode.Linear):
        await message.answer('[Error #303] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>')
        os.remove(texturename)
    ...
    if not texture.Transcode(pvrpy.PixelFormat.ASTC_4x4, pvrpy.VariableType.UnsignedByteNorm, pvrpy.ColourSpace.sRGB,
        ...
        await message.answer('[Error #304] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>')
        os.remove(texturename)

    # sRGB создает потом надо на Linear, а то темно
    if texture.SetTextureColourSpace(pvrpy.ColourSpace.Linear) == False:
        await message.answer('[Error #305] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>')
        os.remove(texturename)

    texturename1 = os.path.splitext(texturename)

    if not texture.SaveToFile(texturename1[0] + ".ktx"):
        await message.answer('[Error #306] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>')
        os.remove(texturename)
...
async def convertprocess_btxtopng(texturename, message: Message):
    ...
    if not texture.Decompress(10):
        await message.answer('[Error #310] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>')
        os.remove(texturename)
    ...
    if not texture.SaveSurfaceToImageFile(texturename1[0] + ".png"):
        await message.answer('[Error #312] Ошибка при конвертации файла! Перешлите это <b>@wellpoper1337</b>')
        os.remove(texturename)
